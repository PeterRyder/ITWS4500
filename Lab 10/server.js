...
app.get('/visualize', function (req, res) {
  ...
  TweetModel.find(function (err, tweets) {
    //console.log(tweets);
    fs.open("cloud.json", "w", function(error, fd) {
      output = {}
      final_output = [];
      for (var tweet in tweets) {
        var split_text = tweets[tweet]["text"].split(" ");
        for (var word in split_text) {
          if (split_text[word] in output) {
            output[split_text[word]]++;
          }
          else {
            output[split_text[word]] = 1;
          }
        }
      }
      ...
      for (var key in output) {
        temp = {}
        temp["text"] = key;
        temp["size"] = output[key];
        final_output.push(temp);
      }

      fs.write(fd, JSON.stringify(final_output, null, 4));
    });

    fs.open("usernames.json", "w", function(error, fd) {
      final_output = [];
      for (var tweet in tweets) {
        temp = {};
        temp["text"] = tweets[tweet]["user_name"];
        temp["size"] = tweets[tweet]["user_followers_count"];
        var found = false;
        for (var i = 0; i < final_output.length; i++) {
          if (final_output[i]["text"] == temp["text"]) {
            found = true;
          }
        }
        if (!found) {
    ...
    fs.open("line.csv", "w", function(error, fd) {

      var output = {};
      var final_output = [];

      for (var tweet in tweets) {
        if (tweets[tweet]["created_at"] in output) {
          output[tweets[tweet]["created_at"]]++;
        }
        else {
          output[tweets[tweet]["created_at"]] = 1;
        }
      }
      ...
      final_output.sort(function(a,b){
      ...
      converter.json2csv(final_output, json2csvCallback);
    });
  });
